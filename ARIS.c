...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// FINITE-CONTEXT MODEL SCHEME
//
typedef struct{
  uint16_t *cnt;     // COUNTERS VECTOR
  uint32_t ctx;      // CONTEXT ORDER FOR THE FCM
  uint32_t aDen;     // DENOMINATOR OF ALPHA
  ...
  uint32_t probs[5]; // REPEAT MODEL SYMBOL PROBABILITIES
  double   weight;   // WEIGHT FCM PERFORMANCE
  uint8_t  rev;      // INVERTED REPEAT USAGE
  }
FCMODEL;
...
typedef struct{
  ...
  uint32_t pos;      // POSITION ON SEQUENCE
  }
ENTRY;
...
//
void ComputeMixture(RCLASS *C, PMODEL *P, uint8_t *b){
  uint32_t n, s;
  ...
  for(n = 0 ; n < C->nRM ; ++n){
    ...
    for(s = 0 ; s < NSYM ; ++s)
      F[s] += C->RM[n].probs[s] * C->RM[n].weight;
    }

  P->sum = 0;
  for(s = 0 ; s < NSYM ; ++s){
    P->freqs[s] = 1 + (uint32_t)(F[s] * MAXC);
    P->sum += P->freqs[s];
    }
  }
...
void Compress(RCLASS *C, char *fn){
  ...
  fprintf(stderr, "Size of DNA sequence: %"PRIu64"\n", pos);
  fprintf(stderr, "Compressed bytes: %u\n", _bytes_output);
  fprintf(stderr, "Ratio: %.4g\n", pos / (double) _bytes_output);
...
int main(int argc, char **argv){
  ...
  if(argc < 2){
    fprintf(stderr, "Usage: ARIS [OPTIONS]... [FILE]                \n"); 
    fprintf(stderr, "                                               \n"); 
